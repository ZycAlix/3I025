...
#               la méthode step(.) de la classe AgentTypeA
...
#           Liste *exhaustive* des informations que vous pouvez utiliser (cf. fonction step(.)):
#               sur soi-même:
...
#           [!!!] les commandes de translation/rotation sont exécutées *après* la fonction step(.). Par conséquent, seules les valeurs passées lors du dernier appel des fonctions setTranslationValue(.) et setRotationValue(.) sont prises en compte!
#
#       Recommandations:
...
#           La manière dont vous construirez votre fonction step(.) est libre. Par exemple: code écrit à la main, code obtenu par un processus d'apprentissage ou d'optimisation préalable, etc.; comportements individuels, collectifs, parasites (p.ex: bloquer l'adversaire), etc.
#
#       Evaluation:
...
class AgentTypeA(object):
    ...
    # =-= JOUEUR A -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= 
    # =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    # =-=-=-=-= le jour de l'évaluation, seul le teamname et la fonction step(.)  =-=
    # =-=-=-=-= seront utilisés. Assurez-vous donc que tout votre code utile est  =-=
    # =-=-=-=-= auto-contenu dans la fonction step. Vous pouvez changer le        =-=
